...
const samplerConfig = [
  ...
  },
  {
    ...
    staticImg: 'gifs/flower-web.png', // Path to static image (first frame)
    audioList: [                       // Random sample picked on each trigger
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addVideoBtn) {
        ...
        // Lazy-load the ~30KB video-sampler module on first click. Saves
        // that JS off the critical path for users who never use the feature.
        // window.createVideoPad is defined by script-video.js once loaded;
        // until then, queued clicks are flushed by that script on load.
        let _videoModuleRequested = false;
        const launchVideoPad = () => {
            if (typeof window.createVideoPad === 'function') {
                window.createVideoPad();
                return;
            }
            window.__videoPadQueue = window.__videoPadQueue || [];
            window.__videoPadQueue.push(1);
            if (_videoModuleRequested) return;
            _videoModuleRequested = true;
            const s = document.createElement('script');
            s.src = 'script-video.js';
            s.async = true;
            document.head.appendChild(s);
        };
        addVideoBtn.addEventListener('click', launchVideoPad);
...
// ─── Video Sampler Pad ───────────────────────────────────────────────────────
// Moved to script-video.js — lazy-loaded on first click of the
// "+ video sampler" button. See the launchVideoPad wrapper above.
