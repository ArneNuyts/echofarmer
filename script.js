...
// External-video sampler pads: each pad has a URL input, scrub bar with cue
// markers, hold-to-play trigger, key binding, volume slider, and a close
// button. Multiple pads can coexist; the "+ video" button spawns new ones.
const _videoPads = [];
let _videoPadIdCounter = 0;
const _defaultKeys = ['v', 'b', 'n', 'm', ',', '.', '/'];

function _nextDefaultKey() {
    const used = new Set(_videoPads.map(p => p.boundKey));
    for (const k of _defaultKeys) if (!used.has(k)) return k;
    return 'v';
}

function setupVideoPads() {
    const container = document.getElementById('video-pad-container');
    if (!container) return;
    const addBtn = document.getElementById('add-video');
    if (addBtn) addBtn.addEventListener('click', () => createVideoPad());

    document.addEventListener('keydown', (e) => {
        const binder = _videoPads.find(p => p.bindingMode);
        if (binder) {
            if (e.key === 'Escape') {
                binder.bindingMode = false;
                binder.keyBtn.classList.remove('binding');
                binder.refreshKeyLabel();
                e.preventDefault();
                return;
            }
            if (['Shift', 'Control', 'Alt', 'Meta'].includes(e.key)) return;
            binder.boundKey = e.key.length === 1 ? e.key.toLowerCase() : e.key;
            binder.bindingMode = false;
            binder.keyBtn.classList.remove('binding');
            binder.refreshKeyLabel();
            e.preventDefault();
        ...
        const isTextField = tag === 'TEXTAREA' ||
            (tag === 'INPUT' && !['range', 'checkbox', 'radio', 'button', 'submit', 'reset'].includes(type));
        if (isTextField) return;
        if (e.repeat) return;
        const k = e.key.length === 1 ? e.key.toLowerCase() : e.key;
        for (const p of _videoPads) {
            if (k === p.boundKey) { e.preventDefault(); p.press(); }
        }
    });
...
function createVideoPad() {
    ...
    const id = ++_videoPadIdCounter;
    const initialKey = _nextDefaultKey();
    const offset = ((_videoPads.length) % 8) * 28;

    const pad = document.createElement('div');
    pad.className = 'video-pad';
    pad.setAttribute('aria-label', 'Video sampler');
    pad.style.left = (50 + offset) + 'px';
    pad.style.top = (50 + offset) + 'px';
    pad.style.right = 'auto';
    pad.style.bottom = 'auto';
    pad.innerHTML = `
        <button class="video-close" type="button" title="Close">&times;</button>
        <input type="text" class="video-url" placeholder="paste video URL (mp4/webm/mov or YouTube)" spellcheck="false" autocomplete="off">
        <div class="video-stage">
            <video class="video-el" preload="metadata" playsinline crossorigin="anonymous"></video>
            <div class="video-yt" hidden></div>
        </div>
        <div class="video-cue-row">
            <div class="video-cue-track" title="Cue points (right-click marker to remove)"></div>
        </div>
        <div class="video-scrub-row">
            <input type="range" class="video-scrub" min="0" max="1000" value="0" step="1" aria-label="Playhead">
            <span class="video-time">0:00 / 0:00</span>
        </div>
        <div class="video-vol-row">
            <span class="video-vol-label">VOL</span>
            <input type="range" class="video-vol" min="0" max="100" value="100" step="1" aria-label="Volume">
            <button class="video-cue-add" type="button" title="Add cue point at current playhead">+ cue</button>
        </div>
        <div class="video-pad-row">
            <button class="video-key" type="button" title="Click then press a key to bind">KEY: ${initialKey.toUpperCase()}</button>
            <button class="video-trigger" type="button">HOLD TO PLAY</button>
        </div>
    `;
    container.appendChild(pad);

    const urlInput = pad.querySelector('.video-url');
    const video = pad.querySelector('.video-el');
    const ytMount = pad.querySelector('.video-yt');
    const scrub = pad.querySelector('.video-scrub');
    const timeLabel = pad.querySelector('.video-time');
    const keyBtn = pad.querySelector('.video-key');
    const trigger = pad.querySelector('.video-trigger');
    const closeBtn = pad.querySelector('.video-close');
    const volSlider = pad.querySelector('.video-vol');

    const state = {
        id, pad, keyBtn, trigger,
        boundKey: initialKey,
        bindingMode: false,
        press: null, release: null, refreshKeyLabel: null, destroy: null
    };

    // --- Drag the whole pad around the viewport ---
    let dragging = false;
    let dragDX = 0, dragDY = 0;
    const isInteractive = (el) => !!(el && el.closest(
        '.video-url, .video-scrub, .video-trigger, .video-key, .video-stage, .video-close, .video-vol'
    ));
    pad.addEventListener('pointerdown', (e) => {
        ...
        // Switch from top/left defaults to explicit positioning, clear right/bottom.
    ...
    const ytCmd = (func, args) => {
        if (!ytPlayer) return;
        try { ytPlayer.sendMessage({ event: 'command', func, args: args || [] }); }
        catch (e) {
            if (ytIframe && ytIframe.contentWindow) {
                try { ytIframe.contentWindow.postMessage(JSON.stringify({ event: 'command', func, args: args || [] }), '*'); }
                catch (e2) {}
            }
        }
    };
    ...
    const setControlsEnabled = (enabled) => {
        ...
        scrub.style.opacity = enabled ? '' : '0.4';
        trigger.disabled = !enabled;
        trigger.style.opacity = enabled ? '' : '0.5';
        trigger.style.cursor = enabled ? '' : 'not-allowed        trigger.styl p        trigger.styl) =        trigger {
                          URL(url);
            const            const       ce            const                    const  utu.be            conhname.slice(1            const      ;
                         Wi                         Wi    it                kie.com                         Wi  athname                         Wi chParams.get('v');
    ...
    const parseInstagram = (url) => {
        try {
            ...
            const host = u.hostname.replace(/^www\./  '');
                                           om')       n null;
            const m             const m             const m       /);
            if (m) return { kind: m[1],            if (m) return { kind: m[1],            iul            if (m) return { kind: m[1],            if (m) return { kind: m[1],            iul            if (m) ) return             if (m) return { kindt pr            if (m) return { kind: m[1],            if (m) return { kind: m[1],            iul            if (m} catch (e) {} resolve(window.YT); };
        if (!document.querySelector('script[data-yt-api]')) {
            const s = document.createElement('script');
            s.src = 'https://www.youtube.com/iframe_api';
            s.async = true;
            s.dataset.ytApi = '1';
                                                                       fmt = (s                f (!isFinite(                                t m =    h.floor(s /                                or(s                                                         0'                                                          el.t                                                                  ubFromTime = (s) => { if (duration > 0) scrub.value = String(Math.round((s / duration) * 1000)); };
    const timeFromScrub = () => duration * (Number(scrub.value) / 1000);

    const tryRouteAudio = () => {
        if (routed || backend !== 'file') return;
        const ctx = getAudioContext();
        if (!ctx) return;
        try {
    ...
    con    con    con    con   
        ...
        ytPollTimer = setInterval(() => {
            if (!ytReady) return;
            if (isHeld) { setScrubFromTime(ytCurrentTime); updateTimeLabel(ytCurrentTime); }
        }, 100);
    };
    ...
    const teardownYt = () => {
        stopYtPoll();
        ytReady = false;
        if (ytMessageHandler) { window.removeEventListener('message', ytMessageHandler); ytMessageHandler = null; }
        if (ytPlayer) { try { ytPlayer.destroy(); } catch (e) {} ytPlayer = null; }        if (ytPlayer) { try { ytPlayer.destroy(); } catch (e) {} ytPl true        if (ytPlayer) { try { };


       if (ytPlayer) { try { ytPlayer.destroy(dow       if (ytPlayer) { try { ytPlayer         if (ytPlayer) { te);
        vide        vide        vide        vide        vide     h (e) {}
        video.src = url;
        duration = 0; startTime = 0;
        scrub.value = '0'; updateTimeLabel(0);
        try { video.load(); } catch (e) {}
        applyVolume();
    };

    const loadYou    const loadYideoId) => {
                      ;
        setControlsEnabled(true);
        try { video.pause(); } catch (e) {}
        video.removeAttribute('src'); try { video.load(); } catch (e) {}
        video.hidden        video.ht.hidden = false;
        duration = 0; startTime = 0; ytCurren        duration = 0; ub.        duration = 0; startTime = 0; ytCurren        duration = 0; ub.        duration = 0; startTime = 0; ytlocatio        duration = 0; startTime = 0; ytCurren        duration = 0; ub.        duration = 0; startTime = 0; ytCurren        duration = 0; ub.        duration = 0; startTime = 0; ytlocatio  nt.i        duration = 0; startTime = 0; ytCurren        duration = 0; ub.        duration = 0; startTime = 0; ytCurreow        duration = 0; startTime = 0; ytCurren        duration = 0; uckgrou        duration = 0;
        ytIframe = ytMount.querySelector('iframe');
        const YT = await loadYouTub        const YT = await loadYouTub      ur        const YT = = fa             ytMessageHandler = (e) => {
            if (!ytIframe || e.source !== ytIframe.contentWindow) retu            if (!ytIframe || e.source !==               if (!ytIframe || e.source !== ytIframe.contentWindow) retutch            if (!ytIframe || e.s if (!data) return;
            if (data.event === 'onStateChange' && typeof data.info === 'number') { ytState = data.info; return; }
            ...
            if (typeof info.currentTime === 'number') {
                ...
                if (isHeld) { setScrubFromTime(ytCurrentTime); updateTimeLabel(ytCurre                if (isHeld) { setScrubFromTime(dow.a                if (isHeld) { setScrubFromTime(ytCurrentTime); updateTimeLabel(ytCurre                if (isHeld) { setScrubFromTime(dow.a() => { ytReady = true; updateTimeLabel(0); startYtPoll(); applyVolume(); },
                onError: (ev) => {
                    const codes = { 2: 'invalid video id', 5: 'playback error', 100: 'video not found / private', 101: 'embedding disabled by uploader', 150: 'embedding disabled by uploader' };
                    timeLabel.textContent = 'YouTub                    timeLabel.textContent = 'YouTub                  if (ev.data === 2 || ev.data === 100                    timeLabel.textContent = 'YouTub                    timeLabel   }
            }
        });
    };

    const loadInstagram = (perma) => {
        teardownYt();
        backend = 'ig';
        setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabo"        setControlsEnabled(false rd        setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(fl         setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setContr          setControlsEnabled(false        setControlsEnabled(false        setl)        set           sadd     Lis        setControlsEnabled(false  nput.        setControls urlInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { loadUrl(urlInput.value.trim()); urlInput.blur(); } });

    video.addEventListener('loadedmetadata', () => { duration = video.duration || 0; updateTimeLabel(0); });
    video.addEventListener('timeupdate', () => { if (isHeld) { setScrubFromTime(video.currentTime); updateTimeLabel(video.currentTime); } });

    const onScrubInput = () => {
        const t = timeFromScrub();
        startTime = t; updateTimeLabel(t);
        if (backend === 'file') {
            if (video.readyState >= 1) { try { video.currentTime = t; } catch (e) {} }
        } else if (backend === 'yt' && ytReady) { ytCmd('seekTo', [t, false]); }
    };
    const commitScrub = () => { if (backend === 'yt' && ytReady) ytCmd('seekTo', [startTime, true]); };
    ...
    scrub.addEventListener('inp    scrub.addEventListener('inp    scrub.addEventListener('inp    scrub.addEventListener('inp    s       scrub.addEventListener('inp    scrub.addEventListener('inp    scrub.addEventListener('inp    scagName ===    scrub.addEventListener('inp    scrub.addEventListener('inp    scrub.addEventListener('inp    scrub.addEventListener('inp    s       scrunst    scrub.addEventListener('inpon * factor;
        const next = Math.max(0, Math.min(duration, startTime + dt)        const next = Math.max(0,ScrubFromTime(next); updateTimeLabel(next);
        if (backend === 'file') {
                                                                                       {} }
                                                                                                             (w                                                                                                        Com                                                       },                                                  =>                                                                                                             (w                                                                                                        Com                                                       },                                                  =>                                                                                                   eo                                                                                                             (w                             );
                                 '                                 '                                 '                    ssList.add('active');
    ...
    const release = () => {
        ...
        if (backend === 'file') { try { video.pause(); } catch (e) {} }
        else if (backend === 'yt') { ytCmd('pauseVideo'); }
        setScrubFromTime(startTime);
        updateTimeLabel(startTime);
    };

    trigger.addEventListener('pointerdown', (e) => { e.preventDefault(); press(); });
    trigger.addEventListener('pointerup', release);
    trigger.addEventListener('pointerleave', releas    trigger.addEventListener('pointerleave', releas    trigger.addEventListener('pointerleave', releas    trigger.addEventListener('pointerleaoU    trigger.addEventListener('pEventListener('click', () => { state.bindingMode = true; keyBtn.cla    trigger.addEventListener('pointerleave', releas   �    trigger.addEventListener('p => {    trigger.addEventListener('pointerleave',         trigger.a
                                 o.                                 o.           (e) {}
          ns          ns          ns  (state);
        if (idx >= 0)         if (idice(idx, 1);
        pad.remove();
    };
    ...
    state.refreshKeyLabel = refreshKeyLabel;
    state.destroy = destroy;
    _videoPads.push(state);
    return state;
}
